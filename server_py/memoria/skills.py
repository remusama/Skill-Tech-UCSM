...
def get_trends(db: Session, user_id: int):
    ...
    last_areas = db.query(ExamResult.area).filter(
        ExamResult.user_id == user_id
    ).order_by(ExamResult.timestamp.desc()).limit(3).all()
    unique_areas = list(set([a[0] for a in last_areas]))

    for area in unique_areas:
        # Average of last 2 vs average of previous 5
        results = db.query(ExamResult.score).filter(
            ExamResult.area == area,
            ExamResult.user_id == user_id
        ).order_by(ExamResult.timestamp.desc()).limit(7).all()
