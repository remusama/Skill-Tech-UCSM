...
def update_eleonor_state(analysis, session):
    ...
    if v_d > 0: session.valence = "positiva"
    elif v_d < 0: session.valence = "negativa"

    session.tension = clamp(session.tension + (t_d * 1.5))
    session.engagement = clamp(session.engagement + (e_d * 1.5))
    
    # Gestión de Boundary
    if session.tension > 0.9: session.boundary = "hold"
    elif session.tension > 0.75: session.boundary = "warning"
    else: session.boundary = "none"

def map_expression(session):
    boundary = session.boundary
    if boundary == "hold": return "Mentira" 
    
    v, t, e = session.valence, session.tension, session.engagement
    
    if t > 0.8: return "Enojo"
    
    if v == "positiva":
        if e > 0.8: return "coqueta"
        if t < 0.4: return "Feliz"
        return "Neutro"
        
    if v == "negativa":
        if t > 0.6: return "Enojo"
        if t < 0.4: return "Tristeza"
        return "Tristeza2"
...
def get_behavioral_mode(session, analysis):
    ...
    if boundary == "hold": return "Baja Interferencia"
    if analysis.get("contradicciones_detectadas") or session.tension > 0.65: return "Soporte Estructurado"
    if session.valence == "negativa" or session.tension > 0.4: return "Empatía Técnica"
    return "Normal"
...
def get_ssml_voice_mode(session):
    ...
    if v == "positiva" and t < 0.4: return "calma_acompañante"
    if e > 0.75 and v == "positiva": return "energia_contenida"
    if t > 0.7: return "directiva_suave"
    if v == "negativa" or t > 0.6: return "reflexiva"
