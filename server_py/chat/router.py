...
from server_py.memoria.database import SessionLocal, ExamResult, EleonorHistory, ChatMessage, EleonorSession, get_db
...
async def generate_response_stream(user_text: str, user_id: int):
    ...
    if "quiero jugar un juego" in user_text.lower():
        llm_messages.append({"role": "system", "content": "ADVERTENCIA: El usuario ha pedido jugar. DEBES iniciar tu respuesta con el tag [GAME] y luego comentar algo sobre el desafío."})
                ...
                try:
                    ...
                    if ana_tag in full_content:
                        try:
                            ...
                            mapped_ana = {}
                            if "v" in ana_json: mapped_ana["valence_delta"] = ana_json["v"]
                            if "t" in ana_json: mapped_ana["tension_delta"] = ana_json["t"]
                            if "e" in ana_json: mapped_ana["engagement_delta"] = ana_json["e"]
