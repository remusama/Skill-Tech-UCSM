...
from server_py.memoria.database import SessionLocal, EleonorSession, ChatMessage, ExamResult, EleonorHistory, get_db
from server_py.eleonor.brain import update_eleonor_state, get_behavioral_mode, map_expression, get_ssml_voice_mode, count_tokens
...
async def websocket_chat(websocket: WebSocket):
    ...
    try:
        ...
        if welcome_data.get("text"):
            ...
            # Reusing existing TTS logic/imports if available or new call
            # For now, let's reuse the same flow as the main loop if we want audio.
            # But welcome_py returns text. Let's just create audio here if we want consistent behavior.
            # To keep it simple as per request (just want text log validation mostly), we send text.
            # User asked "recuerdas lo que eleonor dice un mensaje personalizado... agrega un log".
            
            # Ideally we reuse the TTS generation block:
            try:
        ...
        while True:
            ...
            if message_data.get("type") == "analyze_node":
                ...
                prompt = f"""
                Analiza el siguiente diagnóstico del usuario en el área de {node_name}:
                {diag_context}
                
                Genera un insight breve y motivador (máx 2 frases) que explique su desempeño.
                Usa un tono constructivo y "tech-mystical" (estilo Eleonor).
                """
                
                # We reuse the main flow for consistency (context, memory, voice) by injecting this as a system prompt or user message
                # But to ensure it overrides normal chat logic, let's treat it as a specific user query "Analiza mi nodo X"
                # and inject the system prompt context.
                
                user_text = f"Analiza mi desempeño en {node_name}. [SYSTEM_INJECTED_DATA: {diag_context}]"
                # Fallthrough to main loop to let it process through LLM with session context
                # This ensures we get [TEXTO], [AUDIO], etc. automatically.
            ...
            # Save user message (Solo texto para la DB por ahora, podrías guardar la URL de la imagen si se persiste)
            ...
            if hilo_context: formatted_history.insert(0, {"role": "system", "content": hilo_context})
            ...
            # Re-format history to support multi-modal if needed (GPT-4o accepts mixed roles)
            # Normal history is text-only. The last message (current) might be multimodal
            ...
            async for chunk in stream:
                ...
                if chunk.choices:
                    ...
                    # Hidden chunk log to keep console clean
                    # if delta: print(f"LLM Chunk: {delta}")
                    full_content += delta
            ...
            # Optimization: We can start TTS sooner, but for stability let's do it here first
