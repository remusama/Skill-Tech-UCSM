...
from server_py.memoria.database import ExamResult, User, UserSkill
import openai
...
def get_stats_data(db: Session, classroom: str = None, school: str = None):
    ...
    if student_ids:
        avg_skills = db.query(UserSkill.area, func.avg(UserSkill.level).label("average")).filter(UserSkill.user_id.in_(student_ids)).group_by(UserSkill.area).all()
        ...
        if bloom_count > 0:
            for k in global_bloom: global_bloom[k] = round(global_bloom[k] / bloom_count, 2)
        if vector_count > 0:
            for k in global_vector: global_vector[k] = round(global_vector[k] / vector_count, 2)
    else:
        avg_skills = []
        total_students = 0
...
def get_student_quantum_data(db: Session, student_id: int):
    ...
    exams = db.query(ExamResult).filter(ExamResult.user_id == student_id).order_by(ExamResult.timestamp.desc()).limit(50).all()
    ...
    if energy_val > 80: learning_energy = "Elevada"
    elif energy_val < 40: learning_energy = "Baja / Requiere Estímulo"

    # 2. Academic Risk (grounded in actual performance)
    risk_level = "Bajo"
    if avg_score < 45: risk_level = "Alto"
    elif avg_score < 70: risk_level = "Medio"
    ...
    academic_areas = ["ciencia", "ciencias", "matematicas", "matematica", "humanidades", "ingenieria", "medicina", "logica", "comprension lectora"]
