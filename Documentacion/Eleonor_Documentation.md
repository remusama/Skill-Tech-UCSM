...
- **Responsabilidad**: Procesamiento cognitivo, generación de voz, transcripción de audio, gestión de memoria.
- **Módulos Clave**:
    - `main.py`: API Gateway.
    - `personality.py`: Definición del "Alma" (Prompts y comportamiento).
    - `stt.py`: Oído digital (Whisper).
    - `tts.py`: Voz digital (OpenAI/Edge-TTS).
...
### 🗣️ `tts.py` (Text-to-Speech Dual)
Implementa una estrategia híbrida para la generación de voz:
1.  **Motor Primario (OpenAI)**: Usa el modelo `tts-1` con la voz `nova`.
...
- [ ] **Memoria a Largo Plazo**: Implementar base de datos vectorial (ChromaDB/Pinecone) para que Eleonor recuerde hechos de sesiones pasadas.
