      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isTemplate && <span className="text-[10px] px-2 py-0.5 bg-blue-500/20 text-blue-400 rounded-full font-bold uppercase tracking-wider">Plantilla Base</span>}
        ...
        <div className="flex gap-2 flex-shrink-0">
          ...
          <button
            ...
            className="px-4 py-1.5 rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white text-xs font-bold transition-all shadow-md flex items-center gap-1.5"
          >
  ...
  return (
    <div className="space-y-10 max-w-5xl mx-auto pb-20">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-2 font-medium">Crea, prueba y gestiona agentes evaluadores en tiempo real.</p>
      </div>
            ...
            <div className="p-8 rounded-3xl border border-purple-500/30 bg-gradient-to-br from-purple-500/5 to-blue-500/5 space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Nombre del Agente *</label>
                  <input
                    type="text"
                    value={name}
                    onChange={e => setName(e.target.value)}
                    placeholder="Ej: Evaluador de Pensamiento Crítico"
                    className="w-full h-12 px-4 rounded-xl bg-white/5 border border-white/10 text-white focus:border-purple-500 focus:outline-none"
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Descripción Breve</label>
                  <input
                    type="text"
                    value={description}
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Prompt del Sistema (Instrucción IA)</label>
                <textarea
                  value={systemPrompt}
                  ...
                  rows={3}
                  className="w-full p-4 rounded-xl bg-white/5 border border-white/10 text-white focus:border-purple-500 focus:outline-none text-sm"
                />
              </div>

              <div className="space-y-3">
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Competencias a Evaluar *</label>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 h-11 px-4 rounded-xl bg-white/5 border border-white/10 text-white focus:border-purple-500 focus:outline-none text-sm"
                  />
                  <button onClick={addCompetency} type="button" className="px-5 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-bold transition-colors">
                    Agregar
                  </button>
                </div>
        ...
        {testingAgent && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-2xl bg-[#0F0826] border border-purple-500/30 rounded-3xl p-6 shadow-2xl flex flex-col h-[600px] relative overflow-hidden"
            >
              {/* Modal Header */}
              <div className="flex items-center justify-between pb-4 border-b border-white/10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-purple-300/60 font-medium">Evaluando: {(testingAgent.competencies || []).join(", ") || "General"}</p>
                  </div>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 my-2 custom-scrollbar">
                ...
                {chatLoading && (
                  <div className="flex justify-start">
                    <div className="p-4 rounded-2xl bg-white/10 border border-white/10 text-xs text-purple-300 font-medium animate-pulse flex items-center gap-2">
              ...
              <form onSubmit={handleSendChatMessage} className="pt-3 border-t border-white/10 flex gap-2">
                <input
                  ...
                  className="flex-1 h-12 px-4 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:border-purple-500 focus:outline-none"
                />
                <button
                  type="submit"
                  disabled={chatLoading || !chatInput.trim()}
                  className="px-6 h-12 rounded-xl bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 disabled:opacity-40 text-white font-bold text-sm transition-all"
                >
                  Enviar
                </button>
              </form>
      ...
      {loading ? (
        ...
        <>
          {/* Base Agents */}
          <div>
            <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Agentes Base de Evaluación</h2>
            <div className="space-y-3">
