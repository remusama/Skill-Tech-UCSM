    ...
    <div className="relative min-h-screen w-full flex flex-col pt-6 overflow-hidden bg-gradient-to-b from-[#012216] via-[#023320] via-40% to-[#3c5a21]">
      ...
      <div className="max-w-4xl mx-auto w-full px-6 flex-1 flex flex-col relative z-10">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pb-6 space-y-3">
          {kurtLewinItems.map((item, index) => {
            let bgClass = "bg-[#063924]/60 border-[#0b4a30]";
            if (index >= 12 && index < 21) {
              bgClass = "bg-[#063924]/60 border-[#0b4a30]";
            } else if (index >= 21) {
              bgClass = "bg-[#063924]/60 border-[#0b4a30]"
            }
            ...
            return (
              <Card 
                key={item.id} 
                className={`p-4 backdrop-blur-xl rounded-2xl flex items-center justify-between gap-4 transition-colors ${bgClass}`}
              >
        ...
        <div className="pb-6 flex justify-between items-center gap-4">
            ...
            <Button onClick={handleSubmit} disabled={submitting} className="ml-auto bg-gradient-to-r from-[#032318] to-[#214f3c] text-white font-black uppercase text-xs tracking-widest px-8 py-5 rounded-xl border border-[#063924]">
