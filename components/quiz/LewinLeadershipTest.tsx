      ...
      <div className="max-w-4xl mx-auto w-full px-6 flex-1 flex flex-col relative z-10">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pb-6 space-y-3">
          {kurtLewinItems.map((item) => (
            <Card key={item.id} className="p-4 bg-white/[0.04] border-white/10 rounded-2xl flex items-center justify-between gap-4">
              <div className="flex gap-3 flex-1">
                <span className="text-xs font-black text-[#bf00ff] shrink-0">{item.id}.</span>
              ...
              <div className="flex gap-2 shrink-0">
                {(["A", "D"] as const).map((v) => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-xl text-xs font-black border transition-all ${answers[item.id] === v ? "bg-[#bf00ff] border-[#bf00ff] text-white" : "bg-white/5 border-white/10 text-white/60 hover:bg-white/10"}`}
                  >
                    {v}
                  </button>
                ))}
        ...
        <div className="pb-6 flex justify-between items-center gap-4">
          ...
          <Button onClick={handleSubmit} disabled={submitting} className="ml-auto bg-gradient-to-r from-[#7a00cc] to-[#bf00ff] text-white font-black uppercase text-xs tracking-widest px-8 py-5 rounded-xl">
