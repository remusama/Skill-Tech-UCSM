    ...
    <div className="relative min-h-screen w-full flex flex-col pt-6 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto w-full px-6 flex-1 flex flex-col relative z-10">
        ...
        <div className="flex gap-2 mt-3 items-center flex-wrap">
          {(["M","F"] as const).map((g) => (
            <button key={g} onClick={() => setGender(g)} className={`px-4 py-2 rounded-xl text-xs font-black border ${gender===g?"bg-[#bf00ff] border-[#bf00ff] text-white":"bg-white/5 border-white/10 text-white/60"}`}>{g==="M"?"Varón":"Mujer"}</button>
          ))}
        ...
        <div className="flex gap-2 mt-3 overflow-x-auto pb-1">
          {[0,1,2,3,4,5].map((s) => {
            ...
            return <button key={s} onClick={() => setSection(s)} className={`px-3 py-1.5 rounded-xl text-xs font-black border whitespace-nowrap ${section===s?"bg-white text-black border-white":c===40?"bg-green-500/20 text-green-400 border-green-500/30":c>0?"bg-amber-500/20 text-amber-400 border-amber-500/30":"bg-white/5 border-white/10 text-white/40"}`}>S{s+1} {c}/40</button>
          })}
        </div>
        <div className="flex-1 overflow-y-auto custom-scrollbar pb-6 space-y-3 mt-4">
          {sectionItems.map((item) => (
            <Card key={item.id} className="p-3 bg-white/[0.04] border-white/10 rounded-2xl flex items-center justify-between gap-3">
              <div className="flex gap-2 flex-1"><span className="text-xs font-black text-[#bf00ff]">{item.id}.</span><p className="text-sm text-white/80 leading-snug">{item.text}</p></div>
              <div className="flex gap-1 shrink-0">
                {[0,1,2,3,4].map((v) => (
                  <button key={v} onClick={() => setAnswers((p) => ({ ...p, [item.id]: v as LikertValue }))} className={`w-8 h-8 rounded-lg text-[10px] font-black border ${answers[item.id]===v?"bg-[#bf00ff] border-[#bf00ff] text-white":"bg-white/5 border-white/10 text-white/60"}`}>{["TD","D","N","A","TA"][v]}</button>
                ))}
              </div>
            </Card>
          ))}
        ...
        <div className="pb-6 flex justify-between gap-4 items-center">
          ...
          <div className="flex gap-2 ml-auto">
            <Button variant="outline" disabled={section===0} onClick={() => setSection((s)=>Math.max(0,s-1))} className="border-white/10 text-white">Anterior</Button>
            {section < 5 ? <Button disabled={!canNext} onClick={() => setSection((s)=>s+1)} className="bg-white text-black font-black">{canNext?"Siguiente":"Completa 40/40"}</Button> : <Button onClick={handleSubmit} disabled={submitting || answered<240 || !gender} className="bg-gradient-to-r from-[#7a00cc] to-[#bf00ff] text-white font-black uppercase text-xs px-8 py-5 rounded-xl">{submitting?"Enviando...":"Ver perfil T"}</Button>}
          </div>
        </div>
